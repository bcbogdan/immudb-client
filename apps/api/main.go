...
func setupRouter() *gin.Engine {
	...
	rootDirectory, _ := common.FindProjectRoot()
	fileService := services.NewFileService(rootDirectory)

	r.MaxMultipartMemory = 8 << 20
	accountController := infrastructure.NewAccountController(accountService)
	fileController := infrastructure.NewFileController(fileService)
	...
	r.POST("/file/upload", fileController.UploadFile())
	r.GET("/file", fileController.ListFiles())
